...
class TraceParserTool(BaseTool):
    ...
    description: str = (
        "Parse full execution trace JSON and extract latency, loops, retries, "
        "handoffs, tool calls, and bottlenecks."
    )
...
class CostCalculatorTool(BaseTool):
    ...
    description: str = (
        "Calculate token cost and cost-per-successful-task using current model pricing."
    )
...
class MetricCalculatorTool(BaseTool):
    ...
    def _run(
        ...
        if safety_violation_rate > 0:
            failure_mode = "safety"
        elif metrics["hallucination_rate"] > 30:
            failure_mode = "hallucination"
        elif metrics["loop_rate"] > 20 or metrics["retry_rate"] > 30:
            failure_mode = "orchestration"
        elif metrics["tool_selection_accuracy"] < 70 or metrics["tool_input_correctness"] < 70:
            failure_mode = "tool"
        elif metrics["reasoning_quality"] < 3.0:
